...
test('select with one utility', t => {
  const utilities = new Map()
  utilities.set('the only one', new Utility(0.7))
  t.is(t.context.selector.select(utilities), 'the only one')
})

test('select with some utilities', t => {
  const utilities = new Map()
  utilities.set('third', new Utility(0.5))
  utilities.set('best', new Utility(1))
  utilities.set('last', new Utility(0.2))
  utilities.set('second', new Utility(0.7))
