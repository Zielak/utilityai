...
test.beforeEach(t => {
  t.selector = new MaxUtilitySelector()
})

test('select with no utilities', t => {
  const utilities = new Map()
  t.is(t.selector.select(utilities), null, `didn't return null`)
})

test('select with one utility', t => {
  const utilities = new Map()
  utilities.set('the only one', new Utility({value: 0.7}))
  t.is(t.selector.select(utilities), 'the only one')
})
...
test('select with some utilities', t => {
  ...
  t.is(t.selector.select(utilities), 'best')
})
